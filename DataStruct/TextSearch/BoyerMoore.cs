...
namespace DataStruct.TextSearch
{
    ...
    ///        <1> 若P中  存在  P[k, i] == P[j+1 ,m-1]                即从右往左， 有1个或多个子串与好后缀完全匹配, 则右移子串 m-1-i; 如串 aabcaab  公式:gs[m-1-suff[i]] = m-1-i; 
    ...
    ///                                                                                                                                 
    ///        <2> 若P中  存在  P[k, i] 为 P[j+1 ,m-1] 子集           即从右往左， 有1个或多个子串与好后缀部分匹配, 则右移子串 m-1-i; 如串 abccaab  公式与作用范围同上
    ///                                                                                                                            sufflen 0211007  i即为部分等于好后缀的前缀的右边界
    ///                                                                                                                                 gs 5555571  
    ///                                                                                                                                 
    ///        <3> 若P中 不存在 P[k, i] 为 P[j+1 ,m-1] 子集           即从右往左， 毫无子串与好后缀哪怕部分匹配, 则右移整个模式串 m;  如串 example  
    ///                                                                                                                                    0000007
    ///                                                                                                                                    7777771
